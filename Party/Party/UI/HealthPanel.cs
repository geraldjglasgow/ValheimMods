...
{
    /// <summary>
    /// The draggable party health panel. Dragging only works in edit mode (<c>/party panel edit</c>, PLAN.md
    /// "Health panel drag / ESC") so the window never steals the mouse during normal play.
    /// </summary>
    public static class HealthPanel
    {
        ...
        public static void Draw()
        {
            ...
            rect = GUI.Window(WindowId, rect, DrawWindow, EditMode ? "Party (drag me - /party panel done to finish)" : "Party");
            GUI.color = previous;
            HandleDrag(rect);
        }
